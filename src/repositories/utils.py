...
def rooms_ids_for_booking(date_from: date, date_to: date, hotel_id: int | None = None):
    ...
    get_rooms_ids_for_hotel = select(RoomsORM.id).select_from(RoomsORM)

    if hotel_id:
        get_rooms_ids_for_hotel = get_rooms_ids_for_hotel.filter_by(hotel_id=hotel_id)

    get_rooms_ids_for_hotel_sub: Subquery = get_rooms_ids_for_hotel.subquery(
        name="get_ids_for_hotels"
    )
    ...
    rooms_ids_to_get = (
        ...
        .filter(
            ...
            rooms_left_tabel.c.room_id.in_(get_rooms_ids_for_hotel_sub),  # type: ignore
        )
    )
